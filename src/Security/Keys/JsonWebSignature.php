...
class JsonWebSignature
{
    public static function generate(
        ...
        if ($privateKey === false) {
            throw new AcmeException('Cannot load private key for signing.');
        }
        ...
        $algorithm = match ($keyType) {
            ...
            default => throw new AcmeException('Unsupported key type for JWS signing.')
        };
        ...
        $payload64 = Base64::urlSafeEncode(str_replace('\/', '/', json_encode($payload, JSON_UNESCAPED_SLASHES)));
        ...
        $success = openssl_sign($protected64 . '.' . $payload64, $signed, $privateKey, $opensslAlgorithm);

        if (! $success) {
            throw new AcmeException('JWS signing failed.');
        }
